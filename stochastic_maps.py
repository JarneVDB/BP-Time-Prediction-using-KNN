...
##stochastic map is returned
def create_smap(log, net, im, fm, distribution=None):
    smap = get_map_from_log_and_net(log, net, im, fm, force_distribution=distribution)
    return smap

##stochastic map in the form of a dictionary
def create_distributiondic(smap):
    distributiondic = dict()
    for i in smap:
        k = str(smap[i])
        k = k.replace(" ", ";")
        distributiondic[i] = k.split(";")
        if len(distributiondic[i]) > 1:
            for j in range(1, len(distributiondic[i])):
                distributiondic[i][j] = float(distributiondic[i][j])
    return distributiondic

##stochastic map is created
